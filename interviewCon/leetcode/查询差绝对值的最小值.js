 ...
 */
let nums = [4,6,2,2,7]
let queries = [[0,3],[2,4],[0,4],[0,4],[0,3]]
var minDifference = function (nums, queries) {
    let result = []
    let min
    queries.forEach((item) => {
        let arrItem = nums.slice(item[0], item[1] + 1)
        let newItem  = Array.from(new Set(arrItem))
        if (newItem.length == 0) {
            min = -1
        } else if (arrItem.length > 2) {
            min = 999999999
            for(let i = 0;i<newItem.length;i++){
                for(let j = i+1;j<newItem.length;j++){
                    min = Math.abs(newItem[i]-newItem[j])>min?min:Math.abs(newItem[i]-newItem[j])
                }
            }
            
        } else {
            min = Math.abs(nums[item[0]] - nums[item[1]])
        }
        result.push(min)
    })
    return result
...
console.log(minDifference(nums, queries))
